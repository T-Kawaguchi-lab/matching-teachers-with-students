...
def update_master_title_file(root_dir: Path, append_path: str | Path | None = None) -> Path:
    ...
    if append_path:
        ...
        extra_df = load_master_title(_save_temp_copy(extra_df, root_dir / 'generated' / '_tmp_master_append.xlsx'))
...
def run_pipeline(
    ...
    if append_master_title_path:
        ...
        # initialize from bundled history source if missing
    ...
    for group in GROUPS:
        ...
        group_teachers = teachers[teachers['group'] == group].drop_duplicates(subset=['teacher_name', 'group']).reset_index(drop=True)
        if group_students.empty or group_teachers.empty:
            continue
        similarity = compute_similarity(group_students, group_teachers, model, {
            'field_similarity_weight': float(cfg['field_similarity_weight']),
            'content_similarity_weight': float(cfg['content_similarity_weight']),
        })
        scores_df, rec_df = top_matches_for_group(group_students, group_teachers, similarity, top_k=int(cfg['top_k_rerank']))
    ...
    _export_excel(students_out, {g: students[students['group'] == g] for g in GROUPS if not students[students['group'] == g].empty})
    _export_excel(teachers_out, {g: teachers[teachers['group'] == g] for g in GROUPS if not teachers[teachers['group'] == g].empty})
    _export_excel(rec_out, {g: rec_all[rec_all['group'] == g] for g in GROUPS if not rec_all[rec_all['group'] == g].empty})
    _export_excel(score_out, {g: scores_all[scores_all['group'] == g] for g in GROUPS if not scores_all[scores_all['group'] == g].empty})
    ...
    status = {
        ...
        'groups': {g: {'students': int((students['group'] == g).sum()), 'teachers': int((teachers['group'] == g).sum())} for g in GROUPS},
...
if __name__ == '__main__':
    ...
    print(run_pipeline(teacher_path=args.teacher_path, student_path=args.student_path, history_path=args.history_path, append_master_title_path=args.append_master_title_path))
