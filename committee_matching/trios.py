...
def _search_jglobal_candidates_direct(name: str) -> List[Dict[str, str]]:
    session = requests.Session()
    session.headers.update(DEFAULT_HEADERS)
    base = DEFAULT_JGLOBAL_BASE_URL.rstrip('/')
    name_no_space = normalize_text(name).replace(' ', '')

    url_specs = []
    for template in SEARCH_QUERIES:
        query = template.format(name=normalize_text(name), name_no_space=name_no_space)
        url_specs.extend([
            (f'{base}/search/anythings', {'q': query}),
            (f'{base}/search/researchers', {'q': query}),
            (f'{base}/search/researchers/', {'q': query}),
        ])

    candidates: List[Dict[str, str]] = []
    seen = set()
    for url, params in url_specs:
        try:
            html = fetch(url, params=params, session=session)
        except Exception:
            continue
        soup = BeautifulSoup(html, 'lxml')
        for link in soup.select('a[href*="detail?JGLOBAL_ID="]'):
...
def _search_jglobal_candidates_via_duckduckgo(name: str) -> List[Dict[str, str]]:
    session = requests.Session()
    session.headers.update(DEFAULT_HEADERS)
    queries = [
...
def search_jglobal_candidates(name: str) -> List[Dict[str, str]]:
    combined = _search_jglobal_candidates_direct(name)
    if not combined:
        combined = _search_jglobal_candidates_via_duckduckgo(name)
...
def enrich_teacher_from_jglobal(name: str, cache_dir: str | Path) -> Dict[str, object]:
    ...
    try:
        ...
        ranked = sorted(
            candidates,
            key=lambda row: (
                _score_candidate_for_name(name, row.get('display_name', '')),
                1 if row.get('source') == 'jglobal_search' else 0,
            ),
            reverse=True,
        )

        best_nonempty: Optional[Dict[str, object]] = None
        best_nonempty_score = -1
        tried_urls: List[str] = []
        ...
        for idx, candidate in enumerate(ranked[:10], start=1):
            ...
            if _has_profile_data(result) and score >= 100:
                return result
            if _has_profile_data(result) and score > best_nonempty_score:
