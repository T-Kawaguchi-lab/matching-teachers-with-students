...
def compute_similarity(students: pd.DataFrame, teachers: pd.DataFrame, model, weights: Dict[str, float]) -> SimilarityArtifacts:
    ...
    sf = model.encode(_prefixed(student_field_texts, 'query'), normalize_embeddings=True, show_progress_bar=False)
    tf = model.encode(_prefixed(teacher_field_texts, 'passage'), normalize_embeddings=True, show_progress_bar=False)
    sc = model.encode(_prefixed(student_content_texts, 'query'), normalize_embeddings=True, show_progress_bar=False)
    tc = model.encode(_prefixed(teacher_content_texts, 'passage'), normalize_embeddings=True, show_progress_bar=False)
    ...
    total = weights['field_similarity_weight'] * field_score + weights['content_similarity_weight'] * content_score
    return SimilarityArtifacts(total_score=total, field_score=field_score, content_score=content_score, teacher_teacher_similarity=teacher_teacher_similarity)


def top_matches_for_group(students: pd.DataFrame, teachers: pd.DataFrame, similarity: SimilarityArtifacts, top_k: int = 10) -> tuple[pd.DataFrame, pd.DataFrame]:
    score_rows = []
    recommendation_rows = []
