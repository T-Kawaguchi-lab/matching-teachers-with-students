...
$targetFile = Join-Path $incomingDir "teachers_latest.xlsx"
...
foreach ($line in $statusLines) {
    ...
    if ($line -match 'select_teacher_excel\.ps1$') { continue }
    if ($line -match 'select_teacher_excel\.bat$') { continue }
...
Write-Host "Copied to incoming/teachers_latest.xlsx" -ForegroundColor Green
...
$commitMessage = "Update teachers_latest.xlsx ($timestamp)"
