...
def save_upload(uploaded_file, target: Path):
...
st.markdown('タイトル・概要分野・研究内容・研究分野・細かい研究分野を使って学生側を作成し、TRIOS情報・担当タイトル・研究分野から教員側を作成します。MPPS と MSE は UI 上で切り替えて確認できます。')
...
with st.sidebar:
    ...
    if col1.button('入力ファイルを保存', use_container_width=True):
        saved = []
    ...
    if col2.button('master_title に追加', use_container_width=True):
    ...
    if st.button('アップロード内容で類似度を計算', use_container_width=True, type='primary'):
    ...
    if st.button('incoming/generated を GitHub に push', use_container_width=True):
        try:
    ...
    st.markdown('---')
    st.caption('注: GitHub push は実行環境に push 権限がある場合のみ動作します。GitHub Actions 実行確認まではこの環境ではできません。')

metric_cols = st.columns(4)
metric_cols[0].markdown(f'<div class="metric"><div class="small">現在の表示グループ</div><div style="font-size:1.8rem;font-weight:800">{selected_group}</div></div>', unsafe_allow_html=True)
metric_cols[1].markdown(f'<div class="metric"><div class="small">学生数</div><div style="font-size:1.8rem;font-weight:800">{status.get("groups", {}).get(selected_group, {}).get("students", 0)}</div></div>', unsafe_allow_html=True)
metric_cols[2].markdown(f'<div class="metric"><div class="small">教員数</div><div style="font-size:1.8rem;font-weight:800">{status.get("groups", {}).get(selected_group, {}).get("teachers", 0)}</div></div>', unsafe_allow_html=True)
metric_cols[3].markdown(f'<div class="metric"><div class="small">最終更新</div><div style="font-size:1.1rem;font-weight:700">{status.get("updated_at", "未実行")}</div></div>', unsafe_allow_html=True)
...
with left:
    ...
    if SCORES_CSV.exists():
        ...
        scores_df = scores_df[scores_df['group'] == selected_group].copy()
        student_names = ['すべて'] + sorted(scores_df['student_name'].dropna().unique().tolist()) if not scores_df.empty else ['すべて']
        selected_student = st.selectbox('学生で絞り込み', options=student_names)
        if selected_student != 'すべて':
            scores_df = scores_df[scores_df['student_name'] == selected_student]
...
with right:
    ...
    if not tea_df.empty:
        st.dataframe(tea_df, use_container_width=True, hide_index=True)
    else:
...
for col, path, label in [
    ...
    if path.exists():
        col.download_button(label=label, data=path.read_bytes(), file_name=path.name, mime='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', use_container_width=True)
if SCORES_CSV.exists():
    st.download_button('student_teacher_scores_long.csv', SCORES_CSV.read_bytes(), file_name=SCORES_CSV.name, mime='text/csv', use_container_width=True)
st.markdown('</div>', unsafe_allow_html=True)
